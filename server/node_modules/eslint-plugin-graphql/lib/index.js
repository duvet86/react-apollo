...
function createRule(context, optionParser) {
  ...
  try {
    ...
    for (var _iterator = context.options[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      _loop();
    }
  } catch (err) {
...
var rules = {
  'template-strings': {
    ...
    create: function create(context) {
      return createRule(context, parseOptions);
    }
  },
  'named-operations': {
    meta: {
...
function parseOptions(optionGroup) {
  ...
  if (schemaJson) {
    ...
    throw new Error('Must pass in `schemaJson` option with schema object ' + 'or `schemaJsonFilepath` with absolute path to the json file.');
  }
...
var gqlProcessor = {
  ...
  postprocess: function postprocess(messages) {
    ...
    return (0, _lodash.flatten)(messages).filter(function (message) {
      return (0, _lodash.keys)(rules).map(function (key) {
        return 'graphql/' + key;
...
var processors = (0, _lodash.reduce)(gqlFiles, function (result, value) {
